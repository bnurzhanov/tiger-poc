...
* One local machine with the application and validated inference models installed
...
| 0:35-0:50 | Launch the application once with each manifest. Show both feeds and region overlays side by side, initially empty. | "We launch one instance per setup on this local machine. Each instance loads its configuration and starts inference against its physical camera." |
