"""Typed contracts shared across edge sources, presence rules, and sinks."""
...
@dataclass(frozen=True)
class ProcessEvent:
    """A versioned process event ready for publication to local sinks or Microsoft Fabric."""
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.plant_name is not None:
            event["plantName"] = self.plant_name
        if self.plant_id is not None:
            event["plantId"] = self.plant_id
    ...
    def from_dict(cls, event: dict[str, Any]) -> Self:
        ...
        return cls(
            ...
            event_type=event.get("eventType", "ProcessEvent"),
            ...
            model=event["model"],
            source=event["source"],
            plant_name=event.get("plantName"),
...
class Sink(Protocol):
    """Publication boundary shared by local and cloud sinks."""
